...
def _fill_unknown_actors(rankings: List[Dict]) -> List[Dict]:
        ...
        for idx, drama in enumerate(basic_rankings_list[:20]):  # 只处理前20条
            ...
            if series_id:
                ...
                if cached:
                    ...
                    real_search_context += f"主演列表（按顺序，第一位为女主、第二位为男主）: {actors_str}\n"
  ...
  "emotional_analysis": {{
    ...
    "payoff_scenarios": [
      ...
      {{"name": "家庭矛盾", "value": 17}}
        ...
        # 兜底补全演员（避免前端出现"未知"）
        rankings_data = _fill_unknown_actors(rankings_data)
