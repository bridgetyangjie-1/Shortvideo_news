...
🚨【演员补充规则 - 反幻觉 + 短剧垂类】：
- 仅当检索资料中**明确出现**演员姓名时，才填写 female_lead / male_lead。
- 检索无演员信息时，必须留空字符串 ""，禁止编造。
- **只允许短剧垂类演员**（如徐艺真、曾辉、王道铁、何健麒、马秋元、孙樾、李柯以、王格格等），
  禁止填入传统影视明星（周迅、赵丽颖、杨幂、胡歌、孙红雷、邓超、孙俪等）。
- 禁止填入泛化假名（张伟、王强、赵丽、刘梅、李晓彤等）。
- 检索上下文标注「演员信息缺失」的剧目：female_lead / male_lead 必须留空。
- 演员名必须在检索资料正文中出现，禁止从 platform 字段推断演员。
- 禁止将「红果」「抖音」等平台名写入演员字段。
