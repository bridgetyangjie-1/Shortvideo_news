...
_DETAIL_URL = "https://novelquickapp.com/detail?series_id={series_id}"
_DEFAULT_HEADERS = {
    "User-Agent": (
...
_FEMALE_ROLE_HINTS = ("女", "娘", "妃", "小姐", "夫人", "妻", "妈", "母", "姐", "妹", "婆", "姗", "娇", "妮", "苗", "公主", "皇后")
_MALE_ROLE_HINTS = ("男", "爷", "总", "哥", "夫", "爸", "父", "弟", "叔", "伯", "郎", "少", "王", "将", "侯")
...
def parse_cast_from_html(html: str) -> List[Tuple[str, str]]:
    """从红果 /detail 页面 HTML 解析演职员表，返回 [(演员名, 角色名), ...]。"""
    if not html:
        return []
    pairs: List[Tuple[str, str]] = []
    seen: set[str] = set()
    for actor, role in _CAST_HTML_PATTERN.findall(html):
        actor = actor.strip()
        role = role.strip()
...
class HongguoDetailFetcher:
    ...
    def fetch(self, series_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            url = _DETAIL_URL.format(series_id=series_id)
            ...
            if cast_pairs:
                ...
                if not result:
                    ...
                    result.setdefault("female_lead", leads["female_lead"])
                    result.setdefault("male_lead", leads["male_lead"])

            if result and (result.get("actors") or result.get("studio")):
                logger.info("红果详情页爬取成功: series_id=%s actors=%s", series_id, result.get("actors", [])[:3])
                return result

            return None

        except Exception as exc:
            logger.warning("爬取红果详情页失败 series_id=%s: %s", series_id, exc)
            return None
    ...
    def _extract_from_router_json(self, html: str) -> Optional[Dict[str, Any]]:
        ...
        for actor_key in ("actors", "performer", "cast", "role_list"):
            raw = detail_data.get(actor_key)
            if isinstance(raw, list) and raw:
                if isinstance(raw[0], dict):
                    result["actors"] = [
                        str(item.get("name") or item.get("actor_name") or "")
                        for item in raw
                        if item.get("name") or item.get("actor_name")
                    ]
                else:
