...
### 1.1 项目目标
构建一个**双AI协同**的短剧行业数据采集与分析系统：
- **数据采集(I/O层)**：Kimi (Moonshot) API 联网搜索国内数据（微信/知乎/小红书）
- **数据推理(计算层)**：DeepSeek API 稳定输出JSON格式，无Markdown包裹
- **中枢编排**：Python脚本托管在Git，通过GitHub Actions自动调度
- **静态看板**：自动更新HTML Dashboard，托管在GitHub Pages/Vercel

### 1.2 核心功能
- ✅ 每日自动抓取短剧榜单TOP20（Kimi搜索 → DeepSeek推理）
- ✅ Kimi联网搜索补全缺失字段（主演、厂牌、题材）
- ✅ 小红书备用搜索演员数据
- ✅ DeepSeek稳定输出JSON格式
- ✅ 自动统计演员人气排行TOP10（女频/男频各10人）
- ✅ 多轮搜索策略（3种关键词组合）+ 智能停止
- ✅ 搜索失败时推理补充常见演员
- ✅ 获取行业宏观数据 + 投流指标
- ✅ AI分析生成行业洞察和创新机会点
- ✅ DeepSeek基于当日榜单题材爽点动态反推观众画像
- ✅ 历史数据存储（按日期存档）
- ✅ 时间维度对比（日/周/月环比）
- ✅ 投流风向标（流量洼地、红海预警）
...
| 层级 | 技术组件 | 职责 |
|------|----------|------|
| **数据采集层(I/O)** | Kimi (Moonshot) API | 联网搜索国内数据（微信/知乎/小红书） |
| **数据推理层(计算)** | DeepSeek API | 稳定输出JSON格式，无Markdown包裹 |
| **中枢编排层** | Python + GitHub Actions | 调度双AI、数据清洗、文件更新 |
...
│  ├─────────────────────────────────────────────────────────────────────┤       │
│  │                                                                     │       │
│  │   输入: search_results (Kimi返回的文本)                             │       │
│  │                                                                     │       │
│  │   ┌───────────────────────────────────────────────────────────┐    │       │
│  │   │              DeepSeek API (稳定JSON输出)                  │    │       │
│  │   │  ├── 榜单JSON (TOP20，无Markdown包裹)                     │    │       │
│  │   │  ├── 快讯JSON (5条，60-80字，含来源链接)                  │    │       │
│  │   │  ├── 洞察JSON (2条，爆款归因+买量建议)                   │    │       │
│  │   │  ├── 演员榜JSON (女频TOP10 + 男频TOP10)                  │    │       │
│  │   │  └── 行业数据JSON + 基于榜单的观众画像                   │    │       │
│  │   └───────────────────────────────────────────────────────────┘    │       │
│  │                                │                                    │       │
│  │                    ┌───────────────────┐                            │       │
...
| **Tier 3** | AI推理 | DeepSeek分析 | ⚠️ 参考价值 | 洞察、风向标、观众画像（推理结论） |
...
DeepSeek API (深度分析推理)
    ...
    │   ├── 基于 enriched_rankings 反推 audience_profile
...
**数据质量说明**：
- ⚠️ 洞察、风向标、观众画像 **参考价值**（DeepSeek推理结论，需人工验证）
- ✅ 可作为决策参考，但**不应作为唯一依据**
...
│   ├── graphs/nodes/insights_node.py # DeepSeek推理Tier 3数据
│   ├── graphs/nodes/audience_profile_node.py # DeepSeek反推观众画像
...
| **演员榜** | 女频TOP10 + 男频TOP10 | actors数据 | Tier 2 |
...
| 厂牌排行榜 | 📋 规划中 | 统计各MCN爆款率 |
| ROI实时监控 | 📋 规划中 | 需投流数据源 |
| 受众画像可视化 | ✅ 已接入 | DeepSeek基于当日榜单动态反推 |
