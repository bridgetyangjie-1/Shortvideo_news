...
**✅ 保留规则**：每天必须推送5条行业快讯，每条必须包含100字左右洞察分析。
...
**🚨 行业快讯铁律**：
1. **数量铁律**：必须返回5条快讯（不足5条用行业通用洞察补充）
2. **洞察铁律**：每条必须有insight字段，100字左右的深度分析
   - 行业影响：对产业链的影响
   - 趋势判断：未来发展方向
   - 商业价值：商业机会或风险
3. **时间铁律**：优先今日新闻，不足时补充近7天重要动态

**数据结构更新**：
- DailyNews新增`insight`字段（100字左右深度分析）
- 前端显示insight（左侧紫色边框+💡图标）

### v1.7.12版本（JSON格式修复+演员推理强化）

**industry_node JSON格式修复**：
- 问题：`数值(百分比)`等格式说明符被误解为JSON格式错误
- 修复：改用清晰的字段说明格式
